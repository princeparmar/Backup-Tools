...
func HandleCreateScheduledTask(c echo.Context) error {
	...
	if reqBody.EmailIds != nil {
		// Create a map of email_ids with hardcoded "pending" status
		emailStatusMap := make(map[string]string)
		for _, emailID := range reqBody.EmailIds {
			emailStatusMap[emailID] = "false" // Hardcoded pending status for each email
		}
		memory = map[string]interface{}{
			"email_status": emailStatusMap,
		}
	}
...
func HandleGetScheduledTasksByUserID(c echo.Context) error {
	...
	tasks, err := taskRepo.GetByUserID(database.DB, userID)
	if err != nil {
